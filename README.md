<!-- terminology audit 2026-08-26 · บรรทัดเดิมถูกเก็บไว้เป็น HTML comment ต่อท้ายบรรทัดที่แก้ (ขึ้นต้นด้วย OLD:) · ในตาราง เครื่องหมาย | ของบรรทัดเดิมถูก escape เป็น \| เพื่อไม่ให้ตารางพัง -->

# w07_lab_exam — ข้อสอบปฏิบัติ

**รายวิชา:** 204203 Computer Science Technology
**เวลา:** 2 ชั่วโมง · **12 graded checks ข้อละ 1 คะแนน**
**ขอบเขต:** สัปดาห์ที่ 1–5 — shell, log pipelines, เครื่องมือ Bash + Python, permissions และการคัดลอกอย่างปลอดภัย <!-- OLD: **ขอบเขต:** สัปดาห์ที่ 1–5 — เชลล์, log pipelines, เครื่องมือ Bash + Python, permissions และการคัดลอกอย่างปลอดภัย -->

**สอบในห้องแล็บเท่านั้น** — นั่งที่เครื่องในห้องแล็บ `ssh` เข้า **VM ของตนเอง** <!-- OLD: **สอบในห้องปฏิบัติการเท่านั้น** — นั่งที่เครื่องในห้องแลบ `ssh` เข้า **VM ของตนเอง** -->
...
**1. จากเครื่องในห้องแล็บ ให้ `ssh` เข้า VM ของตนเอง** แล้วทำงานทุกอย่างต่อจากนี้บน VM <!-- OLD: **1. จากเครื่องในห้องแลบ ให้ `ssh` เข้า VM ของตนเอง** แล้วทำงานทุกอย่างต่อจากนี้บน VM -->

```bash
ssh user6805xxxxx@10.110.x.x
```

> เครื่องในห้องแล็บเป็นเครื่องสาธารณะ — **ห้ามสร้างหรือเก็บ SSH private key ไว้บนเครื่องแล็บ** <!-- OLD: เครื่องในห้องแลบเป็นเครื่องสาธารณะ — **ห้ามสร้างหรือเก็บ SSH private key ไว้บนเครื่องแลบ** -->
> key ที่ใช้กับ GitHub อยู่บน VM ของนักศึกษาตั้งแต่ `w00_setup` แล้ว <!-- OLD: key ที่ใช้กับ GitHub อยู่บน VM ของคุณตั้งแต่ `w00_setup` แล้ว -->
...
> ชื่อโฟลเดอร์ต้องลงท้ายด้วย **รหัสนักศึกษาของตนเอง** เพราะ `init.sh` อ่านรหัสจากตรงนี้ <!-- OLD: ชื่อโฟลเดอร์ต้องลงท้ายด้วย **รหัสนักศึกษาของคุณ** เพราะ `init.sh` อ่านรหัสจากตรงนี้ -->

**3. สร้าง submission repository บน GitHub แบบ EMPTY** — ห้ามติ๊ก README, `.gitignore` หรือ License <!-- OLD: **3. สร้าง submission repository บน GitHub แบบ EMPTY** — ห้ามติ๊ก README, `.gitignore` หรือ licence -->

```bash
gh repo create 204203-2026/w07_lab_exam-6805xxxxx --private
```

**4. ชี้ clone นี้ไปที่ repo ของนักศึกษา และขอ scope `workflow` เพิ่ม** <!-- OLD: **4. ชี้ clone นี้ไปที่ repo ของคุณ และขอสิทธิ์ push workflow** -->
...
**5. สร้าง fixture (ชุดข้อมูลข้อสอบ) ของตัวเอง** <!-- OLD: **5. สร้าง fixture ของตัวเอง** -->

```bash
bash init.sh
```

<!-- ADDED: นิยามของคำว่า fixture — เดิมคำนี้ถูกใช้ครั้งแรกที่หัวข้อขั้นที่ 5 โดยไม่มีคำอธิบาย -->
> **fixture** คือชุดข้อมูลสำหรับทำข้อสอบที่ `init.sh` สร้างขึ้นจากรหัสนักศึกษา —
> ไฟล์ log ในโฟลเดอร์ `logs/` (ใช้ใน Part 1 และ Part 2) และไดเรกทอรี `log-tool/`
> ที่ตั้ง permission ผิดไว้โดยตั้งใจ (ใช้ใน Part 3 และ Part 4)
...
**ไฟล์ `.seed` ต้องถูก commit ขึ้นไปด้วย** เพราะตัวตรวจ (grader) ใช้ไฟล์นี้สร้างข้อมูลชุดเดียวกับของนักศึกษาขึ้นมาตรวจซ้ำ <!-- OLD: **ไฟล์ `.seed` ต้องถูก commit ขึ้นไปด้วย** เพราะตัวตรวจใช้ไฟล์นี้สร้างข้อมูลชุดเดียวกับของคุณขึ้นมาตรวจซ้ำ -->
ส่วนไฟล์ log และ `log-tool/` ถูก gitignore ไว้และสร้างใหม่ได้เสมอ <!-- OLD: ส่วนไฟล์ log ถูก gitignore ไว้และสร้างใหม่ได้เสมอ -->
...
`submit.sh` จะรัน `check.sh` ให้อีกรอบ แล้ว commit และ push ขึ้น repo ของนักศึกษา <!-- OLD: `submit.sh` จะรัน `check.sh` ให้อีกรอบ แล้ว commit และ push ขึ้น repo ของคุณ -->
...
| `.seed` | รหัสนักศึกษาของตนเอง — grader ใช้สร้าง fixture ชุดเดิมขึ้นมาใหม่ <!-- OLD: \| `.seed` \| รหัสนักศึกษาของคุณ — ตัวตรวจใช้สร้าง fixture ชุดเดิมขึ้นมาใหม่ \| --> |
...
- fixture ของแต่ละคนสร้างจากรหัสนักศึกษาของตนเอง **ตัวเลขของเพื่อนจึงเป็นคำตอบที่ผิดเสมอ** <!-- OLD: - fixture ของคุณสร้างจากรหัสนักศึกษาของคุณเอง **ตัวเลขของเพื่อนคือคำตอบที่ผิดสำหรับคุณ** -->

## เกณฑ์คะแนน

หมวด required มี 12 checks ข้อละ 1 คะแนน บันทึกใน `results/report.json` <!-- OLD: required 12 checks ข้อละ 1 คะแนน บันทึกใน `results/report.json` -->
ส่วน challenge อีก 4 ข้อบันทึกแยกใน `results/challenge_report.json` และ **ไม่มีทางทำให้คะแนนหลักลดลง**

หลังสอบ ผู้สอนจะสร้าง fixture จาก `.seed` ของนักศึกษาแล้วรัน `check.sh` ตรวจซ้ำอีกครั้ง <!-- OLD: หลังสอบ ผู้สอนจะสร้าง fixture จาก `.seed` ของคุณแล้วรัน `check.sh` ตรวจซ้ำอีกครั้ง -->
...
| `submit.sh` บอกว่า push ถูกปฏิเสธ (`rejected` / `non-fast-forward`) | submission repo ของนักศึกษาไม่ว่างเปล่า ให้ลบแล้วสร้างใหม่แบบ EMPTY → `git remote set-url origin <url ใหม่>` → `bash submit.sh` · **ห้ามสั่ง `git pull`** <!-- OLD: \| `submit.sh` บอกว่า push ถูกปฏิเสธ (`rejected` / `non-fast-forward`) \| submission repo ของคุณไม่ว่างเปล่า ให้ลบแล้วสร้างใหม่แบบ EMPTY → `git remote set-url origin <url ใหม่>` → `bash submit.sh` · **ห้ามสั่ง `git pull`** \| --> |
| `submit.sh` พูดถึง `workflow` scope | `gh auth refresh -h github.com -s workflow` แล้วส่งใหม่ |
| `gh: command not found` หรือ gh ยังไม่ได้ login | `gh auth login` บน VM (เหมือนที่ทำไว้ใน `w00_setup`) <!-- OLD: \| `gh: command not found` หรือ gh ยังไม่ได้ล็อกอิน \| `gh auth login` บน VM (เหมือนที่ทำไว้ใน `w00_setup`) \| --> |
